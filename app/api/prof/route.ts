...
export async function GET() {
  try {
    ...
    const transformedProfessors = professors.map(
      (prof: {
        id: string
        user: { id: string, nom: string; prenom: string; email: string; telephone: string | null; photo: string | null }
        type: string
        ...
        type: prof.type,
        matieresEnseignees: prof.matieresProfesseurs.map((mp: { matiere: { nom: string } }) => mp.matiere.nom),
    ...
    return NextResponse.json({ error: "Failed to fetch professors from database" }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (existingUser) {
      return NextResponse.json({ error: "Email déjà utilisée" }, { status: 400 })
    }
    ...
    return NextResponse.json(
      { error: "Failed to add professor", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    )
  }
}
